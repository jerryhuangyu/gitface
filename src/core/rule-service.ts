...
const normalizeDirectoryForMatch = (directory: string, platform: NodeJS.Platform): string => {
  return CASE_INSENSITIVE_PLATFORMS.has(platform) ? directory.toLowerCase() : directory;
};
...
export const ruleDirectoryMatchesTarget = (
  ...
  const normalizedRuleDirectory = normalizeDirectoryForMatch(ruleDirectory, platform);
  const normalizedTargetDirectory = normalizeDirectoryForMatch(targetDirectory, platform);
