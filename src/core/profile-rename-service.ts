...
export class ProfileRenameService {
  ...
  static create(): ProfileRenameService {
    return new ProfileRenameService(ProfileService.create(), RuleService.create());
  }

  async previewRename(oldName: string, newName: string): Promise<RenamePreview> {
  ...
  async renameProfile(oldName: string, newName: string, force: boolean): Promise<RenameResult> {
    const profile = await this.profileService.renameProfile(oldName, newName, force);
    const rulesUpdated = await this.migrateRulesToRenamedProfile(oldName, newName);
    return {
      profile,
      rulesUpdated,
    };
  ...
  private async migrateRulesToRenamedProfile(oldName: string, newName: string): Promise<number> {
    try {
      const rules = await this.ruleService.listRules();
