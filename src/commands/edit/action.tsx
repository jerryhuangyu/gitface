...
const action: (name: string, options: EditProfileOptions) => Promise<void> = withCommandHandling(
  "command:edit",
  async (name, options) => {
    ...
    if (hasUpdates(options)) {
      ...
      try {
        ...
        if (
          options.json &&
          (error instanceof ProfileNotFoundError || error instanceof InvalidProfileError)
        ) {
          const reason =
...
function hasUpdates(options: EditProfileOptions): boolean {
  return Boolean(options.gitName || options.email || options.signingKey || options.unsetSigningKey);
}
