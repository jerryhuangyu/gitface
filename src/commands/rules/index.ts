...
const command = new Command("rules").description("Manage folder-based profile rules");

command
  .command("apply")
  .argument("[directory]", "Directory to resolve and apply (defaults to current working directory)")
  ...
  .option("--dry-run", "Preview profile application without changing git config")
  .option("--strict", "Treat unmatched rules as failures (exit code 1) for CI gating")
  .option("--json", "Output apply result as JSON")
  .option("--json-envelope", "Output apply result with a unified Result Envelope")
...
command
  ...
  .option("-q, --query <text>", "Filter rules by directory/profile (case-insensitive)")
  .option("--limit <number>", "Limit number of listed rules")
  .option("--health", "Include profile/directory integrity status for each listed rule")
  .option(
    "--concurrency <number>",
...
command
  .command("resolve")
  .argument("[directory]", "Directory to resolve (defaults to current working directory)")
  .description("Resolve the effective folder rule for a directory")
  .option("--strict", "Treat unmatched or missing-profile matches as failures (exit code 1)")
  .option("--json", "Output resolve result as JSON")
  .option("--json-envelope", "Output resolve result with a unified Result Envelope")
...
command
  .command("prune")
  .description("Remove stale folder rules (missing profile by default, optional missing directory)")
  .option("--dry-run", "Preview stale rules without removing them")
  .option("--strict", "Treat unresolved prune findings as failures for CI gating")
  .option("--include-missing-directory", "Also prune rules whose target directory no longer exists")
  .option(
    "--concurrency <number>",
    "Limit concurrent integrity checks to a positive integer (default: 8)",
  )
