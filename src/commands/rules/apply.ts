...
import { buildUseChangePlan, getEffectiveChanges, type UseChangeStep } from "../use/output";
import {
...
const writeApplyFallbackResult = (
  ...
  if (result === "unchanged") {
    if (outputMode === "json-envelope") {
      sendRuleApplyFallbackUnchangedEnvelope(targetDirectory, scope, profile, durationMs, traceId);
      return;
    }
    if (outputMode === "json") {
...
export const applyRuleAction: (
...
) => Promise<void> = withCommandHandling("command:rules:apply", async (directory, options) => {
  ...
  const scopedProfileService =
    scope === "local" ? ProfileService.create({ gitBaseDir: targetDirectory }) : profileService;
  ...
  try {
    ...
    if (!matchedRule && !fallbackProfileName) {
      writeApplyUnmatched(outputMode, targetDirectory, scope, Date.now() - startedAtMs, traceId);
      if (options.strict) {
        process.exitCode = 1;
      }
      return;
    ...
    const resolvedProfileName = resolvedRule ? resolvedRule.profileName : fallbackProfileName;
    ...
    const scopedIdentity = await scopedProfileService.getScopedIdentity(scope);
    const currentIdentity = {
      gitName: scopedIdentity.gitName ?? null,
      email: scopedIdentity.email ?? null,
      signingKey: scopedIdentity.signingKey ?? null,
    };
    const effectiveChanges = getEffectiveChanges(buildUseChangePlan(profile, currentIdentity));
    ...
    if (error instanceof ProfileNotFoundError) {
      reason = await buildProfileNotFoundReason(error.profileName, error.message);
    } else {
      reason = error instanceof Error ? error.message : `Unexpected error ${JSON.stringify(error)}`;
    }
    if (outputMode === "json-envelope") {
      sendRuleApplyEnvelopeError("RULE_APPLY_FAILED", reason, Date.now() - startedAtMs, traceId);
    } else if (outputMode === "json") {
      sendRuleApplyFailedJson(targetDirectory, reason);
    } else {
