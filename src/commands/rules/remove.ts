...
export const removeRuleAction: (directory: string, options: RemoveRuleOptions) => Promise<void> =
  withCommandHandling("command:rules:remove", async (directory, options) => {
    ...
    try {
      if (options.dryRun) {
        ...
        const exists = existingRules.some((rule) => rule.directory === normalizedDirectory);
      ...
      if (outputMode === "json-envelope") {
        sendRuleRemoveSuccessResultEnvelope(normalizedDirectory, Date.now() - startedAtMs, traceId);
        return;
      }
      if (outputMode === "json") {
