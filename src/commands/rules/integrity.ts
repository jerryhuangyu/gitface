...
interface ScanRuleIntegrityOptions {
  ...
  concurrency: number;
  profileExistsCheck?: (service: ProfileService, profileName: string) => Promise<boolean>;
...
async function checkProfileExists(service: ProfileService, profileName: string): Promise<boolean> {
  try {
...
export async function scanRuleIntegrity(
  ...
  const directoryExistsCheck = options.directoryExistsCheck ?? checkDirectoryExists;
  ...
  const { results, workerCount } = await mapWithConcurrency(
    ...
    async (rule) => {
      ...
      if (!profileExistsPromise) {
        profileExistsPromise = profileExistsCheck(profileService, rule.profileName);
  ...
  return {
    ...
    metrics: {
      ...
      uniqueDirectoriesChecked: options.checkDirectory ? directoryExistsCache.size : 0,
