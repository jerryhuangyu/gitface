...
import { parseConcurrency, type RuleIntegrityScanMetrics, scanRuleIntegrity } from "./integrity";
...
const filterRulesByQuery = (rules: RuleSnapshot[], query: string | undefined): RuleSnapshot[] => {
...
const applyLimit = (rules: RuleSnapshot[], limit: number | undefined): RuleSnapshot[] => {
  if (limit === undefined) {
    return rules;
  }
...
const summarizeRuleHealth = (rules: RuleHealthSnapshot[]): RuleHealthSummary => {
  return rules.reduce(
    (acc, item) => {
      acc[item.status] += 1;
      return acc;
...
export async function listRulesAction(options: ListRulesOptions): Promise<void> {
    ...
    if (options.health) {
      ...
      if (report.rules.length === 0) {
        if (options.query?.trim()) {
          console.log(chalk.gray(`No folder rules matched "${options.query.trim()}".`));
          return;
        }
