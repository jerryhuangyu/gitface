...
export const sendProfileCreateDryRunMsg = (profile: Profile, overwrite: boolean): void => {
  const signingKey = profile.signingKey ?? chalk.dim("<unset>");
  const infoIcon = chalk.blue("ℹ");
  const checkIcon = chalk.greenBright("✔");
  ...
  console.log(`${infoIcon} ${chalk.dim("signingKey")}  ${signingKey}`);
  console.log();
  console.log(`${checkIcon} Would save profile ${chalk.green(`'${profile.name}'`)}`);
};
...
export const sendProfileCreateDryRunJson = (profile: Profile, overwrite: boolean): void => {
  console.log(
    JSON.stringify({
      status: "dry-run",
      name: profile.name,
...
export const sendProfileCreateFailedJson = (name: string, reason: string): void => {
  console.log(
    JSON.stringify({
      status: "error",
      name,
