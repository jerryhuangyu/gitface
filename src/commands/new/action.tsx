...
const action: (name: string, options: NewActionOptions) => Promise<void> = withCommandHandling(
  "command:new",
  async (name, options) => {
    ...
    if (hasNewProfileOptions(options)) {
      try {
        ...
        if (
          options.json &&
          (error instanceof ProfileAlreadyExistsError || error instanceof InvalidProfileError)
        ) {
