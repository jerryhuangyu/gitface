...
export const sendDoctorCheckResult = (result: DoctorCheckResult): void => {
  const icon =
    result.status === "pass" ? successIcon : result.status === "warn" ? warningIcon : failureIcon;
...
function toFatalErrors(report: DoctorReport, strict: boolean): ResultEnvelopeError[] {
...
function writeDoctorEnvelope(envelope: ResultEnvelope<DoctorEnvelopeData>): void {
