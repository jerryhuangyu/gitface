...
const command: Command = new Command("import")
  ...
  .option("--atomic", "Validate all entries first; abort the whole import if any entry fails")
  .option("--dry-run", "Validate import without writing profiles")
  .option("--strict", "Exit with non-zero status when any profile fails to import/validate")
  .option("--json", "Output machine-readable import results")
  .option("--json-envelope", "Output import results with unified Result Envelope")
  .action(action);
