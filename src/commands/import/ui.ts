...
export const sendImportExistsWarning = (name: string, dryRun = false): void => {
  ...
  console.warn(`${warningIcon} Profile '${name}' already exists. Use --overwrite to replace.`);
};

export const sendImportFailedMsg = (name: string, reason: string, dryRun = false): void => {
  if (dryRun) {
    console.error(`${failureIcon} [dry-run] Failed to validate '${name}': ${reason}`);
    return;
  }
...
export const sendImportSummary = (successCount: number, failCount: number): void => {
  const failureNote = failCount > 0 ? ` ${chalk.red(`${failCount} failed.`)}` : "";
  console.log();
  console.log(`${successIcon} Imported ${successCount} profiles.${failureNote}`);
};

export const sendImportDryRunSummary = (successCount: number, failCount: number): void => {
  const failureNote = failCount > 0 ? ` ${chalk.red(`${failCount} failed validation.`)}` : "";
  console.log();
  console.log(
...
function writeImportEnvelope(envelope: ResultEnvelope<ImportEnvelopeData | null>): void {
