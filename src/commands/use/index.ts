...
export const command: Command = new Command("use")
  ...
  .option("-q, --query <text>", "Filter profile names when profile arg is omitted")
  .option("--dry-run", "Preview git config changes without writing")
  .option("--json", "Output machine-readable JSON")
  .option("--json-envelope", "Output unified Result Envelope JSON with schema/version metadata")
  .action(action);
