...
import { parseConcurrency, scanRuleIntegrity } from "../src/commands/rules/integrity";
...
describe("rules integrity scan", () => {
  ...
  test("parses concurrency with default and validates positive integer", () => {
    ...
    expect(() => parseConcurrency("0")).toThrow("Concurrency must be a positive integer.");
    expect(() => parseConcurrency("abc")).toThrow("Concurrency must be a positive integer.");
  });
  ...
  test("deduplicates profile and directory checks during scan", async () => {
    ...
    const directoryExistsCheck = vi.fn().mockImplementation(async (directory: string) => {
  ...
  test("skips directory checks when directory scanning is disabled", async () => {
    ...
    const report = await scanRuleIntegrity([{ directory: "/tmp/unused/", profileName: "ghost" }], {
