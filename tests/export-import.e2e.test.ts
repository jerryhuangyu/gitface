...
import { exportProfileCommand, importProfileCommand } from "../src/commands/index";
...
describe("export/import e2e", () => {
  test("emits structured export summary with profiles for export --json", async () => {
    ...
    try {
      ...
      await runCli([exportProfileCommand.command], ["node", "gitface", "export", "--json"]);
      ...
      expect(summary.profiles.map((profile) => profile.name).sort()).toEqual(["personal", "work"]);
    } finally {
      restoreLog();
  ...
  test("emits structured export summary with file path for export <file> --json", async () => {
    ...
    try {
      ...
      const exportedContent = JSON.parse(await fs.readFile(exportFile, "utf8")) as Array<
  ...
  test("emits structured results for import --json", async () => {
    ...
    try {
      ...
      const profiles = await service.listProfiles();
      expect(profiles.map((profile) => profile.name).sort()).toEqual(["personal", "work"]);
    } finally {
      restoreLog();
      if (originalXdg === undefined) {
        delete process.env.XDG_CONFIG_HOME;
  ...
  test("imports all entries for --atomic --overwrite when precheck succeeds", async () => {
    ...
    try {
      ...
      await runCli(
        [importProfileCommand.command],
        ["node", "gitface", "import", importFile, "--atomic", "--overwrite", "--json"],
      );
      ...
      const profiles = await service.listProfiles();
      expect(profiles.map((profile) => profile.name).sort()).toEqual(["personal", "work"]);
      expect(profiles.find((profile) => profile.name === "work")?.gitName).toBe("Work User");
    } finally {
      restoreLog();
      if (originalXdg === undefined) {
        delete process.env.XDG_CONFIG_HOME;
  ...
  test("sets non-zero exit code for import --dry-run --json --strict on validation failures", async () => {
    ...
    try {
      ...
      await runCli(
        [importProfileCommand.command],
        ["node", "gitface", "import", importFile, "--dry-run", "--json", "--strict"],
      );
  ...
  test("emits error envelope and exit code for import --atomic --json-envelope precheck failures", async () => {
    ...
    try {
      ...
      await runCli(
        [importProfileCommand.command],
        ["node", "gitface", "import", importFile, "--atomic", "--json-envelope"],
      );
