...
describe("rename command e2e", () => {
  test("renames an existing profile via CLI", async () => {
    ...
    try {
      ...
      await runCli([renameProfileCommand.command], ["node", "gitface", "rename", "old", "new"]);
  ...
  test("reports overwrite=true for --dry-run --force when target exists", async () => {
    ...
    try {
      ...
      await runCli(
        [renameProfileCommand.command],
        ["node", "gitface", "rename", "old", "new", "--dry-run", "--force", "--json"],
      );
  ...
  test("migrates rules to renamed profile and reports rulesUpdated in json output", async () => {
    ...
    try {
      ...
      await runCli([rulesCommand.command], ["node", "gitface", "rules", "add", projectDir, "old"]);
