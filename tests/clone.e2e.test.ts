...
describe("clone command e2e", () => {
  test("clones an existing profile via CLI", async () => {
    ...
    const logs: string[] = [];
    const restoreLog = vi.spyOn(console, "log").mockImplementation((...args: unknown[]) => {
    ...
    try {
      ...
      await runCli([cloneProfileCommand.command], ["node", "gitface", "clone", "source", "target"]);
  ...
  test("emits dry-run JSON preview without creating target profile", async () => {
    ...
    try {
      ...
      await expect(service.getProfile("target")).rejects.toThrow("Profile 'target' was not found.");
    } finally {
      restoreConsole();
      process.argv = originalArgv;
      if (originalXdg === undefined) delete process.env.XDG_CONFIG_HOME;
  ...
  test("emits dry-run JSON preview with overwrite=true when --force is used", async () => {
    ...
    try {
      ...
      await runCli(
        [cloneProfileCommand.command],
        ["node", "gitface", "clone", "source", "target", "--dry-run", "--force", "--json"],
      );
