...
describe("doctor command e2e", () => {
  test("warns when only local identity exists but global identity is missing", async () => {
    ...
    try {
      ...
      expect(output).not.toContain("Global Git identity is set: Doctor User <doctor@example.com>");
  ...
  test("returns exit code 1 in strict mode when warnings are present", async () => {
    ...
    try {
      ...
      await runCli([doctorCommand.command], ["node", "gitface", "doctor", "--strict"]);
...
`,
    ...
    try {
      ...
      await runCli([doctorCommand.command], ["node", "gitface", "doctor", "--json"]);
      ...
      expect(parsed.checks.every((check) => ["pass", "warn", "fail"].includes(check.status))).toBe(
        true,
      );
      expect(process.exitCode).toBeUndefined();
    } finally {
  ...
  test("emits warning metadata and non-zero exit in strict json mode", async () => {
    ...
    try {
      ...
      await runCli([doctorCommand.command], ["node", "gitface", "doctor", "--strict", "--json"]);
...
`,
    ...
    try {
      ...
      await runCli([doctorCommand.command], ["node", "gitface", "doctor", "--json-envelope"]);
  ...
  test("emits error envelope and exit code 1 in strict json-envelope mode", async () => {
    ...
    try {
      ...
      expect(parsed.errors.some((error) => error.code === "DOCTOR_CHECK_WARN_STRICT")).toBe(true);
      expect(process.exitCode).toBe(1);
    } finally {
      logSpy.mockRestore();
      process.chdir(originalCwd);
