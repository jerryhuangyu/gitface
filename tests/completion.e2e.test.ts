...
describe("completion command e2e", () => {
  ...
  test("limits returned profile names with --limit", async () => {
    ...
    try {
      ...
      await runCli(
        [completionCommand.command],
        ["node", "gitface", "completion", "profiles", "--prefix", "al", "--limit", "2"],
      );
  ...
  test("emits machine-readable payload with --json", async () => {
    ...
    try {
      ...
      await runCli(
        [completionCommand.command],
        ["node", "gitface", "completion", "profiles", "--prefix", "wo", "--limit", "1", "--json"],
      );
  ...
  test("emits empty json payload when no completion matches", async () => {
    ...
    try {
      ...
      await runCli(
        [completionCommand.command],
        ["node", "gitface", "completion", "profiles", "--prefix", "wo", "--json"],
      );
  ...
  test("emits result envelope payload with --json-envelope", async () => {
    ...
    try {
      ...
      expect(payload.code).toBe("COMPLETION_CANDIDATES_OK");
      expect(payload.message).toBe("Completion candidates resolved.");
  ...
  test("sets exit code and writes no output for invalid --limit", async () => {
    ...
    try {
      ...
      await runCli(
        [completionCommand.command],
        ["node", "gitface", "completion", "profiles", "--prefix", "wo", "--limit", "0"],
      );
  ...
  test("sets exit code for unsupported completion topic", async () => {
    ...
    try {
      process.exitCode = undefined;
      await runCli([completionCommand.command], ["node", "gitface", "completion", "invalid-topic"]);
  ...
  test("returns result envelope error for unsupported topic with --json-envelope", async () => {
    ...
    try {
      ...
      expect(payload.message).toBe("Completion topic must be: profiles, commands, rules-commands.");
      expect(payload.data).toBeNull();
      expect(payload.errors).toEqual([
        {
          code: "COMPLETION_TOPIC_UNSUPPORTED",
          message: "Completion topic must be: profiles, commands, rules-commands.",
        },
      ]);
      expect(payload.meta.schemaVersion).toBe("1.0.0");
      expect(process.exitCode).toBe(1);
  ...
  test("prints bash snippet with source-argument guards for profile commands", async () => {
    ...
    try {
      ...
      expect(snippet).toContain("COMP_CWORD -eq 1");
      expect(snippet).toContain('gitface completion commands --prefix "$cur" --limit 50');
      expect(snippet).toContain('gitface completion rules-commands --prefix "$cur" --limit 50');
  ...
  test("prints zsh snippet with source-argument guards for profile commands", async () => {
    ...
    try {
      ...
      const snippet = output.join("");
      expect(snippet).toContain("(( CURRENT == 2 ))");
      expect(snippet).toContain('gitface completion commands --prefix "$PREFIX" --limit 50');
      expect(snippet).toContain('gitface completion rules-commands --prefix "$PREFIX" --limit 50');
  ...
  test("returns top-level command completions for commands topic", async () => {
    const originalExitCode = process.exitCode;
    const output: string[] = [];
    const restoreStdout = captureStdout(output);
    ...
    try {
      ...
      await runCli(
        [completionCommand.command],
        ["node", "gitface", "completion", "commands", "--prefix", "us"],
      );

      expect(output.join("")).toBe("use\n");
      expect(process.exitCode).toBeUndefined();
    } finally {
      process.exitCode = originalExitCode;
      restoreStdout();
  ...
  test("returns rules subcommand completions for rules-commands topic", async () => {
    const originalExitCode = process.exitCode;
    const output: string[] = [];
    const restoreStdout = captureStdout(output);
    ...
    try {
      ...
      await runCli(
        [completionCommand.command],
        ["node", "gitface", "completion", "rules-commands", "--prefix", "re"],
      );

      expect(output.join("")).toBe("remove\nresolve\n");
      expect(process.exitCode).toBeUndefined();
    } finally {
      process.exitCode = originalExitCode;
      restoreStdout();
